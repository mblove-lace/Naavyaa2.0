...
# It tells Django to use a proper plural name in admin (otherwise it might show “Categories”)
...
# NOT a normal Python variable. It's a database coloumn definition. Each field corresponds to a column in the database, and the type of field (CharField, DecimalField, etc.) determines the type of data that can be stored in that column.
    ...
    status = models.CharField(max_length=50, choices=STATUS_TUPLES, default="Published")
...
class Variant (models.Model):
        ...
        def __str__(self):
            return self.name
        def avergae_rating(self):
            return Review.objects.filter(product=self.product, active=True).aggregate(models.Avg('rating'))['rating__avg'] or 0
...
class Order(models.Model):
   ...
   payment_status = models.CharField(max_length=50, choices=PAYMENT_STATUS_TUPLES, default="Processing")
   ...
   saved = models.DecimalField(max_digits=12, decimal_places=2, default=0.00,null=True, blank=True, help_text="Total amount saved")
...
class OrderItem(models.Model):
    ...
    initial_total = models.DecimalField(max_digits=10, decimal_places=2, default=0.00, help_text="Total amount before discount")
...
class Review(models.Model):
    ...
    user = models.ForeignKey(user_models.User, on_delete=models.SET_NULL, null=True, blank=True)
    rating = models.PositiveBigIntegerField(choices=RATING, default=None)
